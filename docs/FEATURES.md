...
| Reporting CLI (`report.py`) | `cost` `doctor` `budget` `audit` `savings` `bands` `resumes` `coldstart` `hygiene` `surface` `session` `outcomes` `pipelines` `errors` `discovery` `floor` `serve` `install-hook` `init` | One command line for all insight | Passive |
